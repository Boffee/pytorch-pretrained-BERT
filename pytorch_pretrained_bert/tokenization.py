...
class BertTokenizer(object):
  ...
  def tokenize_reversible(self, text):
    output_tokens = []
    output_groups = []
    for token, group in zip(*self.basic_tokenizer.tokenize_reversible(text)):
      for sub_token in self.wordpiece_tokenizer.tokenize(token):
        output_tokens.append(sub_token)
        output_groups.append(group)
    return output_tokens, output_groups
...
class BasicTokenizer(object):
  ...
  def tokenize_reversible(self, text):
    """Group non-whitespace separated tokens in list."""
    text = self._clean_text(text)
    text = self._tokenize_chinese_chars(text)
    orig_tokens = whitespace_tokenize(text)
    output_tokens = []
    ...
    for i, token in enumerate(orig_tokens):
      ...
      split_tokens = whitespace_tokenize(" ".join(split_tokens))
      output_tokens.extend(split_tokens)
      output_groups.extend(len(split_tokens) * [i])
